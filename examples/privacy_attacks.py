...
def inference_example():
    ...
    for orig in original_datasets:
        for syn in tqdm.tqdm(synthetic_datasets, "Inference attack eval:"):
            ...
            # Iterate over all columns as secret, the rest as aux_cols \
            # and compute the individual risks
            # In the thesis report, the risk with the highest value was chosen \
            # and added to the results table
            for secret in columns:
                aux_cols = [col for col in columns if col != secret]
                test_sing = InferenceCalculator(
                    original_data,
                    synthetic_data,
                    aux_cols=aux_cols,
                    secret=secret,
