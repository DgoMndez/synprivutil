...
from privacy_utility_framework.plots.plots import (
    correlation_plot_heatmap,
    mutual_information_heatmap,
    plot_all_stats_for_stat,
...
def wasserstein_plot_example():
    ...
    for orig in original_datasets:
        ...
        for i, method in enumerate(WassersteinMethod):
            plt.bar(
                index + i * bar_width,
                all_wasserstein_distances[method],
                bar_width,
                label=method.value,
            )
        ...
        plt.ylabel("Distance")
        plt.title(
            f"Wasserstein, Wasserstein Sample and Sinkhorn Distances for \
                Different Synthetic Datasets (Original: {orig})"
        )
        plt.xticks(index + bar_width / 2, synthetic_datasets)
        plt.legend()
        plt.ylim(
            0, max([max(vals) for vals in all_wasserstein_distances.values()]) * 1.1
...
def ks_test_plot_comparison():
    ...
    for orig in original_datasets:
        ...
        print(
            f"Dataset {orig} with KS-Similarity for each synthetic dataset: \
                copulagan, ctgan, gaussian_copula, gmm, tvae, random"
        )
        print(similarities)
