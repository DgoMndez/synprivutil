...
class WassersteinCalculator(UtilityMetricCalculator):
    ...
    def evaluate(self, metric=WassersteinMethod.WASSERSTEIN, n_samples=500, n_iterations=1):
        """
        Evaluates the Wasserstein distance between the original and synthetic datasets using \
        ...
        - n_iterations: int (default: 1); the number of iterations for sampling when using \
            the sampled method.
        ...
        if metric == WassersteinMethod.SINKHORN:
            ...
            wass_dist = ot.sinkhorn2(
                np.ones((original.shape[0],)) / original.shape[0],
        ...
        if metric == WassersteinMethod.WASSERSTEIN_SAMPLE:
            ...
            print(
                f"Sampled Wasserstein Distance: {sampled_dist}, \
                    with n_samples={n_samples}, n_iterations={n_iterations}"
            )
