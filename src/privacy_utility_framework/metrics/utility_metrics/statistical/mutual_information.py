        ...
        for row in mi_df.columns:
            for col in mi_df.columns:
                mi_df.loc[row, col] = normalized_mutual_info_score(
                    dataset[row].astype(str), dataset[col].astype(str), average_method="arithmetic"
                )
        return mi_df

    def evaluate(self):
        """
        Evaluates the similarity in mutual information (MI) structure between original and \
            synthetic datasets.

        Returns:
        - float; the average similarity score based on mutual information between original and \
            synthetic datasets.
        """
