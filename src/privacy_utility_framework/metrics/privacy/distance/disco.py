...
class DisclosureCalculator(PrivacyMetricCalculator):
    ...
    def _disclosure(self, synthetic, original, keys, target):
        ...
        if isinstance(synthetic, pd.DataFrame):
            ...
            raise ValueError(
                f"No disclosure method associated with class {type(synthetic).__name__}"
            )

    def _disclosure_dataframe(
        self, synthetic, original, keys, target, print_flag=True, compare_synorig=True
    ):
        ...
        if compare_synorig:
            adjust_data = self._synorig_compare(
                synthetic if m == 1 else synthetic[0], original, print_flag=False
            )
            if not adjust_data.get("unchanged", True):
                synthetic = adjust_data["syn"]
                original = adjust_data["orig"]
                print(
                    "Synthetic data or original or both adjusted with synorig.compare to try to \
                        make them comparable\n"
                )
                if m > 1:
                    print(
                        "Only the first element of the list has been adjusted and \
                            will be used here\n"
                    )
                    m = 1
    ...
    def _synorig_compare(self, syn, orig, print_flag=True):
        ...
        if print_flag:
            print(
                f"{len_common} variables in common out of {len(syn.columns)} in syn out of \
                    {len(orig.columns)} in orig"
            )
        ...
        for i in range(len_common):
            if pd.api.types.is_numeric_dtype(syn.iloc[:, i]) and isinstance(
                orig.iloc[:, i].dtype, pd.CategoricalDtype
            ):
                syn.iloc[:, i] = syn.iloc[:, i].astype("category")
                nch_syn += 1
                unchanged = False
            if isinstance(
                syn.iloc[:, i].dtype, pd.CategoricalDtype
            ) and pd.api.types.is_numeric_dtype(orig.iloc[:, i]):
                orig.iloc[:, i] = orig.iloc[:, i].astype("category")
                nch_orig += 1
                unchanged = False
        if not unchanged:
            print(
                f"\nVariables changed from numeric to factor {nch_syn} in syn {nch_orig} in \
                    original\n"
            )
        ...
        if not unchanged2:
            print(
                f"\nVariables changed from character to factor {nch_syn} in syn \
                    and {nch_orig} in orig\n"
            )

        # Check data types match in common variables
        for i in range(len_common):
            if pd.api.types.is_integer_dtype(syn.iloc[:, i]) and pd.api.types.is_float_dtype(
                orig.iloc[:, i]
            ):
                syn.iloc[:, i] = pd.to_numeric(syn.iloc[:, i])
                print(
                    f"{syn.columns[i]} changed from integer to numeric in synthetic \
                        to match original"
                )
                unchanged = False
            elif pd.api.types.is_integer_dtype(orig.iloc[:, i]) and pd.api.types.is_float_dtype(
                syn.iloc[:, i]
            ):
                orig.iloc[:, i] = pd.to_numeric(orig.iloc[:, i])
                print(
                    f"{orig.columns[i]} changed from integer to numeric in original \
                        to match synthetic"
                )
                unchanged = False
                ...
                print(
                    f"\nDifferent classes for {syn.columns[i]} in syn: {syn.iloc[:, i].dtype} \
                        in orig: {orig.iloc[:, i].dtype}\n"
                )
                needsfix = True
        ...
        for i in range(len_common):
            if not orig.iloc[:, i].isna().any() and syn.iloc[:, i].isna().any():
                print(
                    f"\n\nMissing data for common variable {syn.columns[i]} in syn but not in \
                        orig\n"
                ...
                unchanged = False
            if isinstance(syn.iloc[:, i].dtype, pd.CategoricalDtype) and isinstance(
                orig.iloc[:, i].dtype, pd.CategoricalDtype
            ):
                ...
                if len(lev1) != len(lev2) or not np.array_equal(lev1, lev2):
                    ...
                    syn.iloc[:, i] = pd.Categorical(
                        syn.iloc[:, i], categories=all_levels, ordered=False
                    )
                    orig.iloc[:, i] = pd.Categorical(
                        orig.iloc[:, i], categories=all_levels, ordered=False
                    )
                    unchanged = False
        if needsfix:
            print(
    ...
    def _disclosure_synds(self, object, data, keys, target: str, print_flag=True):
        ...
        if not (
            all(key in data.columns for key in keys)
            and all(key in names_syn for key in keys)
            and target in data.columns
            and target in names_syn
        ):
        ...
        if "target" in data.columns:
            raise ValueError(
                "your data have a variable called 'target'; \
                    please rename in original and synthetic data."
            )
        ...
        for jj in range(object.m):
            ...
            if print_flag:
                print(
                    f"Table for target {target} from GT alone with keys has \
                        {tab_ktd.shape[0]} rows and {tab_ktd.shape[1]} columns."
                )
            ...
            if not np.all(
                np.isin(Td, Ts)
            ):  # Some original target levels not found in synthetic data
            ...
            if print_flag:
                print(
                    f"Table for target {target} from GT & SD with all key combinations has "
